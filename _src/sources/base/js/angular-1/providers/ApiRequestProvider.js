        ...
        this.$get = [
            ...
            function(
                ...
                var resolveUrl = function(input) {
                    var usePort = input.indexOf(':') == 0;
                    var parser = document.createElement('a');

                    parser.href = input;

                    
                    var pathname = parser.pathname.split('/').filter(_in => _in.length > 0);

                    if (!usePort && (pathname[0] !== ''))
                        pathname.unshift('');

                    return parser.protocol + '//' + parser.hostname + pathname.join('/');
                }
