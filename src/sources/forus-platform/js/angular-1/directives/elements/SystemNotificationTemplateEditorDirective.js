...
const NotificationTemplateEditorDirective = function (
    ...
    const editTemplate = () => {
        ...
        const submitUpdate = (form) => {
            ...
            const newTemplate = {
                ...{ title: labelsToVars(title), content: labelsToVars(content), content_html: labelsToVars(content_html) },
    ...
    $dir.updateTemplatePreview = (template) => {
        ...
        if (template.type === 'mail') {
            $dir.content_preview_sce = $sce.trustAsHtml(labelsToBlocks(
                contentToPreview(template?.content_html || '', $dir.variableValues),
                $dir.implementation
            ));
        }
    }
